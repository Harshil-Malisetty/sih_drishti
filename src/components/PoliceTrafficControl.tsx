...
function TrafficControlDetail({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  return <div className="police-workflow"><AppHeader title="Traffic control" subtitle={id} onBack={onBack}/><main className="page detail">
    <section className="operation-panel" aria-label="Police traffic workflow">
      ...
      <p><strong>Fleet observation candidate</strong> — an elevated traffic ratio is not an authoritative emergency declaration. Officer qualification and manual traffic dispatch are separate actions.</p>
      ...
      {!dispatch && anomaly.status === 'Candidate' && <div className="operation-actions"><button className="primary" disabled={busy || !observation} onClick={() => run(() => policeTrafficService.qualify(id, 'Traffic operator'))}>Flag for dispatch</button></div>}
      {!dispatch && anomaly.status === 'Qualified' && <div className="operation-actions"><button className="primary" disabled={busy || !observation} onClick={() => run(() => policeTrafficService.requestDispatch(id, 'Traffic operator'))}>Request dispatch</button></div>}
      {dispatch?.stage === 'Requested' && <div className="operation-form">
        ...
        <div className="operation-actions"><button className="primary" disabled={busy || !teams.some(item => item.id === teamId) || !assignee.trim()} onClick={() => run(() => policeTrafficService.assign(id, teamId, assignee.trim()))}>Assign traffic team</button></div>
      </div>}
      {dispatch?.stage === 'Assigned' && <div className="operation-actions"><button className="primary" disabled={busy} onClick={() => run(() => policeTrafficService.advanceDispatch(dispatch.id, 'En route', 'Traffic operator'))}>Mark en route</button></div>}
      {dispatch?.stage === 'En route' && <div className="operation-actions"><button className="primary" disabled={busy} onClick={() => run(() => policeTrafficService.advanceDispatch(dispatch.id, 'On scene', 'Traffic operator'))}>Mark on scene</button></div>}
      ...
      {dispatch?.stage === 'On scene' && <>
        ...
        <div className="operation-actions"><button className="secondary" disabled={busy || !observation || observation.baselineVehicleCount <= 0 || recovered} onClick={() => run(() => policeTrafficService.recordFollowUp(id, 'Traffic operator'))}>{recovered ? 'Normalizing follow-up recorded' : 'Record demo follow-up pass'}</button></div>
        ...
        <div className="operation-form">
          ...
          <div className="operation-actions"><button className="primary" aria-describedby={followUpHintId} disabled={busy || !recovered || !summary.trim() || !resultingCondition.trim()} onClick={() => run(() => workflowService.submitDemoResolution(event, summary.trim(), resultingCondition.trim()))}>Mark resolved</button></div>
        </div>
      </>}
      ...
      {dispatch?.stage === 'Verified' && <>
        ...
        <div className="operation-actions"><button className="primary" disabled={busy} onClick={() => run(() => policeTrafficService.close(dispatch.id, 'Traffic operator'))}>Close event</button></div>
      </>}
