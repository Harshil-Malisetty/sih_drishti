...
type Marker={id:string;type:'bus'|'incident'|'watchlist'|'defect'|'water'|'traffic';x:number;y:number;label:string;detail?:string};
export function MapView({markers,selected,onSelect,mode='operations',children}:{markers:Marker[];selected?:string;onSelect?:(id:string)=>void;mode?:string;children?:React.ReactNode}){
 const icons:any={bus:BusFront,incident:ShieldAlert,watchlist:TriangleAlert,defect:Wrench,water:Droplets,traffic:TriangleAlert};
 return <div className={cx('map-view',mode)}>
  ...
  {markers.map(m=>{const Icon=icons[m.type];return <button aria-label={m.label} className={cx('marker',m.type,selected===m.id&&'selected')} style={{left:`${m.x}%`,top:`${m.y}%`}} onClick={()=>onSelect?.(m.id)} key={m.id}><Icon/></button>})}
...
export function BottomSheet({eyebrow,title,children,action,onAction}:{eyebrow?:string;title:string;children:React.ReactNode;action?:string;onAction?:()=>void}){return <div className="bottom-sheet"><i className="handle"/>{eyebrow&&<span className="sheet-eyebrow">{eyebrow}</span>}<h3>{title}</h3>{children}{action&&<button className="primary" onClick={onAction}>{action}</button>}</div>}
