...
export function PotholeLifecycle({defect}:{defect:RoadDefect}){
 ...
 return <section className="lifecycle-chart" aria-labelledby="lifecycle-title">
  <header><div><span>REPEATED FLEET OBSERVATIONS</span><h2 id="lifecycle-title">Pothole Lifecycle</h2></div><strong>{defect.detectionCount}<small>observations</small></strong></header>
  <div className="lifecycle-plot">
   <span className="axis-label y">Estimated condition severity</span>
   ...
   <svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
    ...
    <line className="threshold-line" x1="8" y1="39" x2="92" y2="39"/>
  ...
  <div className="lifecycle-dates" aria-hidden="true">{observations.map(observation=><span key={observation.day}>{observation.date}</span>)}</div>
  <div className="lifecycle-evidence" aria-live="polite">
   ...
   <div><span>{selected.date} · DAY {selected.day}{selectedIndex===observations.length-1?' · CURRENT STATE':''}</span><h3>{selected.label}</h3><p>{selected.detail}</p><small><BusFront/> {selected.busId} · observation {selectedIndex+1} of {observations.length}</small></div>
  </div>
 </section>;
}