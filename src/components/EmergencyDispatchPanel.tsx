...
function EmergencyDispatchWorkflow({ event, actor }: { event: EventRef; actor: string }) {
  ...
  const station = dispatch ? state.policeStations[dispatch.stationId] : undefined;
  ...
  return <section className="operation-panel emergency-panel" aria-label="Emergency response">
    <div className="operation-heading"><h2>Emergency response</h2><span className="operation-state">{dispatch?.stage || 'Automatic allocation'}</span></div>
    <p role="status" aria-live="polite">{busy ? 'Saving emergency response action…' : resolvedAt ? 'Event resolved after team release.' : dispatch ? `Emergency response: ${dispatch.stage}.` : 'Activate once. The nearest police station is assigned automatically.'}</p>
    <p className="operation-meta">Demo only: illustrative station locations. No real emergency services are contacted.</p>
    {!dispatch ? <>
      <div className="operation-actions"><button className="primary" disabled={busy} onClick={() => run(
        () => emergencyService.request(event, 'Emergency response activated for this event', actor),
        ({ station: allocated }) => ({ message: `Emergency team allocated from ${allocated.name}, the nearest police station. Demo only; no real service contacted.`, tone: 'info' }),
      )}>{busy ? 'Allocating emergency team…' : 'Activate emergency response'}</button></div>
    </> : <>
      <p><strong>Emergency team automatically allocated from {station?.name || 'the assigned police station'}.</strong></p>
      <p><strong>Request reason:</strong> {dispatch.reason}</p>
      <dl className="operation-facts">
        <div><dt>Nearest police station</dt><dd>{station?.name || 'Station unavailable'}</dd></div>
        <div><dt>Distance at allocation</dt><dd>{dispatch.distanceKm.toFixed(2)} km · approximate straight-line distance, not a travel estimate</dd></div>
        <div><dt>Allocated</dt><dd>{formatDemoDate(dispatch.assignedAt)}</dd></div>
      ...
      {dispatch.stage === 'Discharged' && <>
        <p>{station?.name || 'Assigned station'} emergency response released{dispatch.dischargedAt ? ` · ${formatDemoDate(dispatch.dischargedAt)}` : ''}.</p>
