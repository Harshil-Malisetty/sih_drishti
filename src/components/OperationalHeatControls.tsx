...
export function OperationalHeatControls({ role, display, points, cases, history, setHistory, repeats, setRepeats, onFit }: {
	role: 'police' | 'municipal'; display: ReturnType<typeof useHeatDisplay>; points: HeatPoint[]; cases?: number; history: boolean; setHistory: (value: boolean) => void;
	...
	return <section className="operational-heat-controls" aria-label="Geographic heatmap controls">
		<div className="operational-heat-toolbar">
			<label className="operational-heat-switch"><input type="checkbox" checked={display.visible} onChange={event => display.setVisible(event.target.checked)}/><span><strong>Hotspot intensity</strong><small>{display.points.length} {history ? 'recorded' : 'active'} road detections{cases === undefined ? '' : ` · ${cases} ${role === 'police' ? 'incident' : 'issue'} ${cases === 1 ? 'record' : 'records'}`}</small></span></label>
			...
			<details className="operational-heat-options"><summary>Hotspot layers</summary><div className="operational-heat-settings"><fieldset><legend>Combine categories</legend>
				...
				<p>{repeats ? 'Lifetime repeat counts are logarithmically weighted, not treated as separate issues. ' : 'Canonical cases carry one weighted sample each. '}The field is generated from the mapped road network; record tabs and vehicle positions remain independent of it.</p>
			</div></details>
		</div>
		<p className="operational-map-note">Simulated city-wide detection distribution across the mapped road network—not severity, predicted risk, or where a bus happens to be. Colours rescale with filters; nearby samples blend and accumulate. {!display.points.length && 'No matching heat samples.'}</p>
	</section>;
}
