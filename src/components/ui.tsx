...
export function FilterBar({items,active,onChange,label='Filter records'}:{items:string[];active:string;onChange:(x:string)=>void;label?:string}){return <div className="filterbar" role="group" aria-label={label}>{items.map(x=><button type="button" aria-pressed={x===active} className={x===active?'active':''} onClick={()=>onChange(x)} key={x}>{x}</button>)}</div>}
