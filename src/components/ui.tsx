...
const SessionActionsContext=createContext<(()=>void)|undefined>(undefined);
export function SessionActionsProvider({onLogout,children}:{onLogout:()=>void;children:ReactNode}){return <SessionActionsContext.Provider value={onLogout}>{children}</SessionActionsContext.Provider>}
...
export function AppHeader({title,subtitle,onBack,onExit}:{title:string;subtitle?:string;onBack?:()=>void;onExit?:()=>void}){const logout=useContext(SessionActionsContext);return <header className="app-header">{onBack?<button className="icon-btn" onClick={onBack} aria-label="Go back"><ArrowLeft/></button>:<div className="seal">D</div>}<div className="header-copy"><strong>{title}</strong><span>{subtitle||'Systems operational'}</span></div><span className="header-demo">DEMO</span>{(onExit||logout)?<button className="exit" onClick={onExit||logout} aria-label="Log out">Logout</button>:null}</header>}
