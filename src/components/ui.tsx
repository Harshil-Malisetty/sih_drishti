...
export function AppHeader({title,subtitle,onBack,onExit}:{title:string;subtitle?:string;onBack?:()=>void;onExit?:()=>void}){return <header className="app-header">{onBack?<button className="icon-btn" onClick={onBack} aria-label="Back"><ArrowLeft/></button>:<div className="seal">D</div>}<div className="header-copy"><strong>{title}</strong><span>{subtitle||'Systems operational'}</span></div>{onExit?<button className="exit" onClick={onExit}>Switch</button>:<button className="notif" aria-label="Notifications"><span/></button>}</header>}
...
export function BottomNavigation({items,active,onChange}:{items:any[];active:string;onChange:(id:string)=>void}){return <nav className="bottom-nav">{items.map(({id,label,icon:Icon})=><button key={id} className={active===id?'active':''} onClick={()=>onChange(id)}><Icon/><span>{label}</span></button>)}</nav>}
