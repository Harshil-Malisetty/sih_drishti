...
import { CityMapStatus, fitOperationalPoints, jumpCamera, useCityMap, useMapMarkers, type CityMapCamera } from './MapLibreMap';
...
export function MunicipalMapView({markers,selected,initialView,onSelect,onViewChange,heatPoints=[],heatOpacity=0,heatFitRequest=0}:{markers:MunicipalGeoMarker[];selected?:string;initialView?:MunicipalMapCamera;onSelect:(id:string)=>void;onViewChange:(camera:MunicipalMapCamera)=>void;heatPoints?:HeatPoint[];heatOpacity?:number;heatFitRequest?:number}){
 ...
 const {handle,loading,error}=useCityMap(container,{initialView:initialView||{center:[13.005,80.238],zoom:12},onViewChange,focus:active?[active.latitude,active.longitude]:undefined});
 useGLHeat(handle,heatPoints,heatOpacity,heatFitRequest);
 useMapMarkers(handle,'city-records',markers.map(marker=>({...marker,color:marker.closed?'#37705b':marker.type==='infrastructure'?'#6e7040':marker.type==='pedestrian'?'#d07b2e':'#7b6136'})),selected,onSelect);
 useEffect(()=>{
