...
function EvidenceView({id,onBack}:{id:string;onBack:()=>void}){
 ...
 return <div className="police-workflow"><AppHeader title="Evidence" subtitle={x.id} onBack={onBack}/><main className="page detail"><h1>{x.type}</h1>
  ...
  <EvidenceCredit src={image} additionalSources={stages.flatMap(stage=>stage.image?[stage.image]:[])}/>
  ...
  {!x.citizenReportId&&x.plateImage&&<figure className="plate-evidence"><EvidenceImage src={x.plateImage} alt="Number-plate reference photo"/><figcaption>Plate reference</figcaption><EvidenceCredit src={x.plateImage}/></figure>}
...
function MatchDetail({id,onBack}:{id:string;onBack:()=>void}){
 ...
 const person=x.subjectType==='Missing Person';
 ...
 return <div className="police-workflow"><AppHeader title="Possible watchlist match" subtitle={x.id} onBack={onBack}/><main className="page detail">
  ...
  <p className="watchlist-person-name">{x.subjectName}</p>
  <p className="evidence-context">Sample comparison</p>
  <div className={`compare watchlist-compare ${person?'person-compare':'vehicle-compare'}`}><figure><div className="comparison-media"><EvidenceImage src={x.referenceImage} alt={person?'Reference crop of the woman in a patterned sari':'Reference crop of the silver car with its front number plate'}/></div><figcaption>{person?'PERSON REFERENCE':'VEHICLE REFERENCE'}</figcaption></figure><div className="comparison-arrow" aria-hidden="true">→</div><figure><div className="comparison-media"><EvidenceScene src={selectedImage} alt={person?'Group photo with the reference woman highlighted':'Street photo with the reference car highlighted'} label={person?'PERSON':'VEHICLE'}/></div><figcaption>SUBJECT IN SCENE</figcaption></figure></div>
  <EvidenceCredit src={selectedImage} additionalSources={[x.referenceImage]}/>
  ...
  {observations.length>0&&<><SectionHeader title="Observation trail"/><div className="evidence-trail">{observations.map((o,index)=><button key={o.id} aria-pressed={selected?.id===o.id} onClick={()=>setObservation(o.id)}>{o.image&&<EvidenceImage src={o.image} alt="" loading="lazy"/>}<span><strong>{index+1}. {o.location} · {formatDemoDate(demoDate(o.timestamp))}</strong><small>{o.busId} · Route {o.route}</small><small>{o.confidence===undefined?'Confidence unavailable':`${o.confidence}% confidence`}</small></span><ArrowRight aria-hidden="true"/></button>)}</div>
  ...
  <div className="trail-summary"><strong>{new Set(observations.map(item=>item.busId)).size} buses</strong><span>Observation history</span></div>
  </>}
  {error&&<p role="alert">{error}</p>}
