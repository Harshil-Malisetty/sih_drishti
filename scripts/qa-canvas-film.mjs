...
try{
  ...
  await sharp({create:{width:1440,height:SCENES.length*270,channels:3,background:'#171b38'}}).composite(snapshots.map((input,i)=>({input,left:(i%3)*480,top:Math.floor(i/3)*270}))).png().toFile(resolve(out,'contact-sheet.png'));
  checks.push(`${snapshots.length} story stills across ${SCENES.length} focused chapters.`);
  ...
  for(const frame of [0,240,820,1550,2010,2250,2980,3600,4260,4570,5399]){const before=await hash(frame);await hash(2333);await hash(19,false);assert.equal(await hash(frame),before,`Nondeterministic frame ${frame}`);assert.notEqual(await hash(frame,false),before,`Caption toggle ignored at ${frame}`);}
  assert.equal(await hash(TOTAL_FRAMES-20),await hash(TOTAL_FRAMES-1),'Final end card must hold');
  checks.push('Eleven shuffled/repeated seeks are byte-identical; clean/caption variants differ; final hold is stable.');
  ...
  assert.equal(timing.frames,TOTAL_FRAMES);checks.push(`All ${TOTAL_FRAMES} frames draw without exceptions; every caption boundary and two-line limit checked.`);
  ...
  await page.focus('#seek');await page.keyboard.press('End');assert.equal(await page.$eval('#film',e=>e.dataset.frame),String(TOTAL_FRAMES-1));await page.keyboard.press('Home');assert.equal(await page.$eval('#film',e=>e.dataset.frame),'0');
  await page.click('#chapters button:nth-child(5)');assert.equal(await page.$eval('#film',e=>e.dataset.scene),'waterlogging');await page.click('#restart');assert.equal(await page.$eval('#film',e=>e.dataset.frame),'0');
